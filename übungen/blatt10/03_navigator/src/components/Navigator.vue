<template>
  <div class="container">
    ...
    <div class="right">
      ...
      <div v-else>
        <h3>Links</h3>
        <a :href="href" :key="href" target="_blank" v-for="href in data.data[data.selectedTop][data.selectedLeft].references">
          {{ href }}
        </a>
      </div>
    </div>
...
onMounted(async () => {
  try {
    ...
    await store.dispatch('setData', json);

    const topKey = Object.keys(json)[0];
    _pushHistory(topKey, Object.keys(json[topKey])[0]);
    isLoading.value = false;
  } catch (e) {
    alert('Error while trying to fetch data: ' + e);
  }
});

const clickUpdateSelection = async (key: string, position: 'top' | 'left') => {
  switch (position) {
    case 'top':
      await store.dispatch('setTop', key);
      _pushHistory(key, data.value.selectedLeft);
      break;
    case 'left':
      await store.dispatch('setLeft', key);
      _pushHistory(data.value.selectedTop, key);
      break;
  }
};

const _pushHistory = (top: string, left: string) => {
  history.pushState({
    top,
    left
  },
...
onMounted(() => {
  window.addEventListener('popstate', (e) => {
    if (e.state) {
      store.dispatch('setBoth', e.state);
    }
  });
});
